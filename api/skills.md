...
- **Error handling contract**
  All JSON endpoints return failures using a unified structure:
  ```json
  {
    "error": {
      "code": "IDENTITY_NOT_FOUND",
      "message": "RODiT identity not found",
      "details": { "tokenId": "pkncjdbdefcp" }
    },
    "requestId": "01HX7HANWC0VCG6F1D2Z1KQ7W4",
    "timestamp": "2026-04-15T07:46:00.000Z"
  }
  ```
  The `code` field is stable for contract tests (see `HELLO_*`, `IDENTITY_*`, etc.), while `message` is human-readable and `details` is optional context.
...
- **get_auth_params**  
  ...
  - Error codes: `RATE_LIMIT_EXCEEDED` (429) when anonymous agents exceed 100 calls/minute, `AGENT_AUTH_PARAMS_FAILED` (500) if entropy generation fails.
...
- **get_jsonld_context**  
  DID:wba JSOat vocabulary for RODiT tokens. Defines semantic mappings for all token metadata fields and Distinguished Name attributes, enabling interoperability with other DID systems.  
...
- **list_agents**  
  ...
  - Error codes: `AGENT_DISCOVERY_FAILED` (500) whenever the upstream NEAR query or decoding pipeline fails.
