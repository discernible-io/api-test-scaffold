...
const { roditManager, stateManager, login_server } = require("./middleware/rodit");
...
async function enhancedClient(config) {
  ...
  try {
    ...
    if (testExecutionState.isRunning) {
      logger.warnWithContext("Enhanced client already running, skipping new execution", logContext);
      return;
    }
    ...
    await roditManager.initializeVault().catch(error => {
      logger.errorWithContext("Vault initialization error, continuing with defaults", logContext, error);
    });
    ...
    try {
      ...
      logger.errorWithContext("RODIT config initialization error", logContext, error);
      throw new Error(`Failed to initialize RODIT configuration: ${error.message}`);
    }
    ...
    if (loginResult.jwt_token) {
      ...
      const TEST_CLIENT_DURATION = parseInt(config?.API_OPTIONS?.TEST_CLIENT_DURATION, 10) * 1000 || 60000;
      const TEST_INTERVAL = parseInt(config?.API_OPTIONS?.TEST_INTERVAL, 10) * 1000 || 5000;
      const MAX_CONCURRENT_TESTS = parseInt(config?.API_OPTIONS?.MAX_CONCURRENT_TESTS, 10) || 1;
      ...
      logger.infoWithContext(
        `Enhanced client will run tests for ${TEST_CLIENT_DURATION / 1000} seconds`,
        testContext
      );
      ...
      const enabledSuites = config?.API_OPTIONS?.ENABLED_TEST_SUITES || Object.keys(testSuites);
      ...
      for (const suiteName of enabledSuites) {
        if (testSuites[suiteName]) {
          ...
          for (const [testName, testFn] of Object.entries(suiteTests)) {
            ...
            if (!excludedTests.includes(fullTestName) && !excludedTests.includes(testName)) {
              filteredTestSuites[suiteName][testName] = testFn;
            } else {
              logger.infoWithContext(`Skipping excluded test: ${fullTestName}`, {
                ...testContext,
...
logger.infoWithContext("Comparative test results", {
  ...
  details: comparativeResult.details || {}
      ...
      logContext.completedTestIterations = testExecutionState.lastCompletedIteration;
      logContext.testSuccessRate = `${((testExecutionState.lastCompletedIteration / testCount) * 100).toFixed(2)}%`;
      logContext.status = "completed";
      
      testExecutionState.endTime = Date.now();
      
      logger.infoWithContext("Enhanced client finished running tests", logContext);
    } else {
...
async function runTestSuite(apiEndpoint, suiteName) {
  ...
  try {
    ...
    if (!testModules[suiteName]) {
      logger.errorWithContext(`Test suite '${suiteName}' not found`, suiteContext);
      return {
        success: false,
    ...
    logger.errorWithContext(`Error running test suite '${suiteName}'`, suiteContext, error);
...
async function runSingleTest(apiEndpoint, suiteName, testName) {
  ...
  try {
    ...
    // Check if suite exists
    if (!testModules[suiteName]) {
      logger.errorWithContext(`Test suite '${suiteName}' not found`, testContext);
      return {
        success: false,
        error: `Test suite '${suiteName}' not found`
      };
    ...
    if (!testModules[suiteName][testName]) {
      logger.errorWithContext(`Test '${testName}' not found in suite '${suiteName}'`, testContext);
      return {
        success: false,
    ...
    logger.infoWithContext(`Running test: ${suiteName}.${testName}`, testContext);
    ...
    logger.infoWithContext(`Test ${suiteName}.${testName} completed`, testContext);
    ...
    logger.errorWithContext(`Error running test '${testName}' in suite '${suiteName}'`, testContext, error);
...
function getTestExecutionState() {
  return {
    ...
    runningDuration: testExecutionState.startTime ? 
      Math.floor((Date.now() - testExecutionState.startTime) / 1000) : 0,
