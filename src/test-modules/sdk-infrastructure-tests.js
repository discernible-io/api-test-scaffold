...
const { extractApiErrorInfo, isMissingNearCredentialError } = require("./test-utils");
...
module.exports = {
  ...
  testSdkRoditClientInitialization: async (api_ep) => {
    try {
      ...
      if (isMissingNearCredentialError(error)) {
        logger.warn("NEAR credentials not available for RODiT init; skipping client assertions", {
          component: "TestRunner",
          method: "testSdkRoditClientInitialization",
        });
        return { passed: true, details: { api_ep, skipped: true, reason: "missing_near_credentials" } };
      }
    ...
    try {
      ...
      if (isMissingNearCredentialError(error)) {
        logger.warn("RoditClient test instance requires NEAR credentials; skipping", {
          component: "TestRunner",
          method: "testSdkRoditClientInitialization",
        });
        return { passed: true, details: { api_ep, skipped: true, reason: "missing_near_credentials" } };
  ...
  testSdkRoditClientProtocolHandling: async (api_ep) => {
    try {
      ...
      const errorInfo = extractApiErrorInfo(error);
      if (isMissingNearCredentialError(error)) {
        logger.warn("NEAR credentials not available; skipping protocol check", {
          component: "TestRunner",
  ...
  testSdkRoditClientCreateInitializes: async (api_ep) => {
    try {
      ...
      if (isMissingNearCredentialError(error)) {
        logger.warn("RoditClient.create smoke test skipped: NEAR credentials not configured", {
          component: "TestRunner",
          method: "testSdkRoditClientCreateInitializes",
        });
        return { passed: true, details: { api_ep, skipped: true, reason: "missing_near_credentials" } };
      }
      throw error;
