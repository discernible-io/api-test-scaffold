 ...
 *     "details": { ... }  // Optional (e.g. reasonCode for HOLA validation)
 *   },
...
/**
 * True when failure is due to missing NEAR credentials (SDK roditmanager requires implicit_account_id).
 * @param {Error|unknown} error
 * @returns {boolean}
 */
function isMissingNearCredentialError(error) {
  const msg = error && error.message ? error.message : String(error);
  return (
    msg.includes("Credentials must contain implicit_account_id") ||
    msg.includes("Credentials not available for") ||
    msg.includes("Error 244: Invalid or missing implicit_account_id") ||
    (msg.includes("SDK initialization failed") && msg.includes("implicit_account_id"))
  );
}
