...
const performanceTests = {
  ...
  testTokenValidationLatency: async (apiEndpoint, logContext) => {
    ...
    try {
      ...
      const sequentialAvg = sequentialDurations.reduce((sum, val) => sum + val, 0) / sequentialDurations.length;
      ...
      const concurrentAvg = concurrentDurations.reduce((sum, val) => sum + val, 0) / concurrentDurations.length;
      ...
      return {
        ...
        details: {
          ...
          comparison: {
            ...
            percentChange: ((concurrentAvg - sequentialAvg) / sequentialAvg * 100).toFixed(2) + '%'
          }
        }
      };
    } catch (error) {
  ...
  testRateLimitBoundaries: async (apiEndpoint, logContext) => {
    ...
    try {
      ...
      const boundaryBehaviorCorrect = boundaryRequest?.success && 
      ...
      return {
        ...
        error: !boundaryBehaviorCorrect ? "Rate limit boundary behavior not as expected" : null,
        details: {
          maxRequests,
          windowMinutes,
