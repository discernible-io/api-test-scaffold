...
const securityTests = {
  ...
  testWebhookSecurity: async (apiEndpoint, logContext) => {
    ...
    try {
      ...
      const webhookUrl = webhookEndpointInfo.webhookUrl || "https://dev-webhook.aparejos.net/webhook";
      ...
      const privateKeyHex = config.own_rodit.private_key || 
      ...
      const invalidSignature = signature.substring(0, signature.length - 2) + 
      ...
      const expiredSignature = Buffer.from(expiredSignatureBytes).toString('hex');
      ...
      const allTestsPassed = validAccepted && invalidRejected && tamperedRejected && expiredRejected;
      
      return {
        success: allTestsPassed,
        error: !allTestsPassed ? "One or more webhook security tests failed" : null,
        details: {
          validAccepted,
          invalidRejected,
          tamperedRejected,
  ...
  testTamperedTokens: async (apiEndpoint, logContext) => {
    ...
    try {
      ...
      const headerJson = JSON.parse(Buffer.from(tokenParts[0], 'base64').toString());
      const tamperedHeader = {
        ...headerJson,
        alg: headerJson.alg === "HS256" ? "RS256" : "HS256" // Tamper with algorithm
      };
      
      const tamperedHeaderBase64 = Buffer.from(JSON.stringify(tamperedHeader)).toString('base64')
        .replace(/=/g, '');
      ...
      const payloadJson = JSON.parse(Buffer.from(tokenParts[1], 'base64').toString());
      const tamperedPayload = {
        ...payloadJson,
        permissions: "admin" // Attempt to elevate permissions
      };
      
      const tamperedPayloadBase64 = Buffer.from(JSON.stringify(tamperedPayload)).toString('base64')
        .replace(/=/g, '');
      ...
      const signatureTamperedToken = `${tokenParts[0]}.${tokenParts[1]}.${tokenParts[2].substring(0, tokenParts[2].length - 3)}abc`;
      ...
      return {
        success: allRejected,
        error: !allRejected ? "System did not reject all tampered tokens" : null,
        details: {
          headerRejected,
          payloadRejected,
          signatureRejected,
  ...
  testReplayAttacks: async (apiEndpoint, logContext) => {
    ...
    try {
      ...
      if (renewalResult.error || !renewalResult.jwt_token) {
        return {
          success: false,
          error: `Token renewal failed: ${renewalResult.error || "No new token received"}`,
  ...
  testPermissionClaims: async (apiEndpoint, logContext) => {
    ...
    try {
      ...
      const restrictedRejected = !!restrictedResult.error && 
                                  restrictedResult.error.includes("permission");
      
      return {
        success: restrictedRejected,
        error: !restrictedRejected ? "System did not properly enforce permission restrictions" : null,
        details: {
          authorizedResult: { 
