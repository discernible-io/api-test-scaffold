...
/** Must match `api-docs/target-swagger.json` `info` (runtime GET /openapi.json contract). */
const TARGET_OPENAPI_INFO_TITLE = 'IdClawPassport API';
const TARGET_OPENAPI_INFO_VERSION = '0.9.5';
...
async function testSwaggerJsonSchema(apiEndpoint, logContext) {
  ...
  try {
    ...
    // Contract: info.title and info.version match api-docs/target-swagger.json
    try {
      const response = await fetch(`${apiEndpoint}/openapi.json`, {
        method: 'GET',
      });
      ...
      const data = await response.json();
      const titleOk = data.info?.title === TARGET_OPENAPI_INFO_TITLE;
      const versionOk = data.info?.version === TARGET_OPENAPI_INFO_VERSION;
      const passed = response.status === 200 && titleOk && versionOk;

      results.push({
        name: 'OpenAPI info matches target-swagger.json',
        passed,
        statusCode: response.status,
        expectedTitle: TARGET_OPENAPI_INFO_TITLE,
        actualTitle: data.info?.title,
        expectedVersion: TARGET_OPENAPI_INFO_VERSION,
        actualVersion: data.info?.version,
      });
    } catch (error) {
      const errInfo = extractApiErrorInfo(error);
      results.push({
