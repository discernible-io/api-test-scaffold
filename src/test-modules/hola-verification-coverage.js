...
const normalizeReasonCode = (reasonCode) => {
  ...
  // target-swagger.json: early HOLA shape / transport lint (identity/verify & testhola)
  const shapeLintAsFormat = new Set([
    'payload_is_json_object',
    'unix_millis_timestamp',
    'signature_not_base32',
    'placeholder_signature',
  ]);
  if (shapeLintAsFormat.has(reasonCode)) {
    return 'invalid_format';
  }
  return reasonCode;
};
...
const extractFailure = (error) => {
  ...
  const reasonCode = normalizeReasonCode(
    details.reasonCode || normalizeErrorCodeToReasonCode(info.code),
  );
  return {
