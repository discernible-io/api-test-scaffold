...
export async function verify_peerrodit_getrodit_fe(
  ...
  try {
    ...
    const peer_rodit = await nearorg_rpc_tokenfromroditid(
      peerroditid,
      authService?.rpcUrl  || "https://rpc.testnet.fastnear.com/",
      authService?.contractId  || "rodit-org.testnet"
    );
 ...
 * This requires HTTPS context in production environments.
 */
  ...
  async login_server_withnep413(options) {
    ...
    try {
      ...
      if (!own_rodit || !own_rodit.token_id) {
        throw new Error("No RODiT found for this account");
      }
    ...
    try {
      ...
      try {
        ...
        if (error.message && error.message.includes('returned null from blockchain')) {
          ...
          logger.error('[validate_jwt_token_fe] Please verify that the server RODiT token exists on the NEAR testnet', {
            component: 'validate_jwt_token_fe'
          });
        }
...
export async function nearorg_rpc_tokenfromroditid(roditid, rpcUrl = null, contractId = null) {
  ...
  // Use provided parameters or fallback to constants
  const finalRpcUrl = rpcUrl || "https://rpc.testnet.fastnear.com/";
  let finalContractId = contractId || "rodit-org.testnet";
...
export async function verifyRoditBeforeMinting(Metadata, SignerPublicKey, type) {
  ...
  const Valid = await verifyRoditSignature(rodit, SignerPublicKey, "server");
