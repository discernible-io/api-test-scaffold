...
const PACKAGE_JSON_PATHS = ["package.json", "sdk/package.json"];
...
function main() {
  const dependencies = PACKAGE_JSON_PATHS.flatMap((packageJsonPath) => {
    const absolutePath = path.join(process.cwd(), packageJsonPath);
  ...
  for (const dependency of dependencies) {
    ...
    try {
      ...
      console.error(
        `[minimum-package-age] Failed to evaluate ${dependency.name}@${dependency.range} in ${dependency.packageJsonPath}: ${error.message}`,
      );
      process.exit(1);
    }
  }
  ...
  if (tooNew.length > 0) {
    ...
    for (const dependency of tooNew) {
      console.error(
        `  - ${dependency.packageJsonPath}: ${dependency.name}@${dependency.version} (${dependency.section}) published ${dependency.publishedAt.toISOString()} (${dependency.ageHours.toFixed(1)}h old)`,
      );
    }
  ...
  console.log(
    `[minimum-package-age] OK. All dependencies in ${PACKAGE_JSON_PATHS.join(", ")} are at least ${MINIMUM_AGE_DAYS} days old.`,
  );
}
