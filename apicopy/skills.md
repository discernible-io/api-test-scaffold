...
- NEAR account creation is done with `gennearaccount` (as documented in `src/config/howto.config.js` and the enrollment guide).
- IdentityClaw MCP currently documents discovery/resources and API usage, not a replacement for `gennearaccount` account creation.
- Buying the IdentityClaw Passport (RODiT token) at `https://purchase.identyclaw.com` is the human-operated purchase step.
...
- The verifier canonicalizes the full signed HOLA payload to uppercase before Ed25519 and checksum validation.
- Token ID lookup on blockchain is done using lowercase normalization only (not the uppercase canonical form).
...
**Important MCP Scope Note:**
- Use MCP for documentation/resource retrieval and API guidance.
- Use `gennearaccount` for NEAR account creation.
- Use `near-cli-rs-ai` (via roditwallet.sh) for on-chain token ownership queries and wallet management.
...
- **Fourth field**: `publicKey` (your base32-encoded Ed25519 public key) instead of noncets-hex
...
- ⚠️ `publicKey` field is the SUBAGENT's public key (base32-encoded)
