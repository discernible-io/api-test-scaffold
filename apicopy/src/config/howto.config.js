...
module.exports = {
  ...
  agentInstructions: {
    ...
    mandatoryBehavior: [
      ...
      "❌ DO NOT use npm, brew, or build from source for near-cli-rs",
  ...
  howToUseSteps: [
    {
      ...
      description: "⚠️ IMPORTANT: You MUST create your NEAR implicit account (wallet) BEFORE purchasing the IdentityClaw Passport (RODiT token). Install near-cli-rs and create an implicit NEAR account.",
      details: {
        command: "near-cli-rs account create-account-using-seed-phrase fund-my-account seed-phrase seed-phrase-length-12 network mainnet",
...
print(f'Verification result: {is_valid}')  # True if value matches`,
      ...
      discoveringYourTokenId: {
        ...
        method1_directBlockchainQuery: {
          ...
          recipe1_findYourTokens: {
            ...
            command: `near-cli-rs contract call-function as-read-only \\
  ...
  now`,
            ...
            example: {
              ...
              fullCommand: `near-cli-rs contract call-function as-read-only \\
  2026v1-identyclaw-com.near \\
  rodit_tokens_for_owner \\
...
result = subprocess.run([
    'near-cli-rs', 'contract', 'call-function', 'as-read-only',
    '2026v1-identyclaw-com.near',
    'rodit_tokens_for_owner',
...
print(f"Private key (64 bytes): {private_key_bytes.hex()}")`
          ...
          recipe3_checkAccountBalance: {
            ...
            command: `near-cli-rs account view-account-summary \\
  YOUR_ACCOUNT_ID \\
  network-config mainnet \\
  now`,
            ...
            example: {
              accountId: "43d3c5b5e77a46b52933bc7a8b79b06f16dd4ca3cfbacd0e6fede0e7e01782ac",
              fullCommand: `near-cli-rs account view-account-summary \\
  ...
  "metadata": {
    ...
    "extra": "{\\"facial_features\\":{\\"eye_distance\\":0.5,\\"nose_width\\":0.3,\\"mouth_height\\":0.4}}"
          ...
          recipe6_createNewAccount: {
            ...
            command: `near-cli-rs account create-account \\
  fund-later \\
  use-auto-generation \\
  save-to-folder ~/.near-credentials/mainnet`,
            
            note: "This creates credentials locally but the account doesn't exist on-chain until funded with at least 0.01 NEAR",
            
            followUp: "To fund: near-cli-rs tokens FUNDING_ACCOUNT send-near NEW_ACCOUNT '0.01 NEAR' network-config mainnet sign-with-legacy-keychain send"
          },
  ...
  ...
        ...
        workflow: {
          ...
          quickCommand: {
            ...
            command: `near-cli-rs contract call-function as-read-only 2026v1-identyclaw-com.near rodit_tokens_for_owner json-args '{"account_id": "YOUR_ACCOUNT_ID"}' network-config mainnet now`,
...
login_payload = {"timestamp": ts_unix, ...}  # ✅ Same moment!`
    ...
    prerequisites: {
      title: "Prerequisites",
      requirements: [
        "near-cli-rs installed (via official installer, not npm/brew)",
        "NEAR tokens available for funding (≥0.01 NEAR for account init, ~1 NEAR for RODiT)",
        "Access to write to $HOME/.near-credentials/"
      ]
    },

    step1_installNearCli: {
      title: "Step 1: Install near-cli-rs",
      description: "Install the official NEAR CLI tool (Rust implementation) using the pre-compiled binary installer",
      
      whyThisApproach: {
        title: "🤖 Why We Use the Official Installer (Not npm/brew/source)",
        reasons: [
          "✅ Official pre-compiled binaries - no build time, guaranteed compatibility",
          "✅ Rust implementation is faster and more reliable than Node.js version",
          "✅ Automatic updates via cargo - stays current without manual version management",
          "✅ Works on all major platforms (Linux, macOS, Windows)",
          "❌ npm packages: outdated, slower, not maintained",
          "❌ Homebrew: often lags behind official releases",
          "❌ Building from source: slow, requires Rust toolchain, error-prone"
        ]
      },
      ...
      installation: {
        ...
        step1_downloadAndRun: {
          description: "Download and run the official installer from GitHub releases",
          command: "curl --proto '=https' --tlsv1.2 -LsSf https://github.com/near/near-cli-rs/releases/download/latest/near-cli-rs-installer.sh | sh",
          note: "Uses 'latest' tag - automatically gets the newest stable version"
        },
        
        step2_updatePath: {
          description: "Add near-cli-rs to your PATH",
          command: "source $HOME/.cargo/env",
          note: "This command adds $HOME/.cargo/bin to PATH. Run this in every new shell, or add to ~/.bashrc or ~/.zshrc"
        },
        
        step3_verifyInstallation: {
          description: "Verify the installation worked",
          command: "near --version",
          expectedOutput: "near-cli-rs X.Y.Z (where X.Y.Z is the version number)",
          note: "If 'near: command not found', run 'source $HOME/.cargo/env' again"
        }
      },
      ...
      verifyCurrentState: {
        ...
        checks: [
          {
            check: "Binary exists",
            command: "ls -la $HOME/.cargo/bin/near",
            expectedResult: "File exists with executable permissions (-rwxr-xr-x)",
            troubleshooting: "If not found, re-run the installer and check for errors"
          },
          {
            check: "Version check",
            command: "near --version",
            expectedResult: "Output shows 'near-cli-rs' followed by a version number (e.g., 0.26.0, 0.27.0, etc.)",
            troubleshooting: "If 'command not found', run: source $HOME/.cargo/env"
          },
          {
            check: "PATH configured",
            command: "which near",
            expectedResult: "Shows path like $HOME/.cargo/bin/near (exact path depends on your $HOME)",
            troubleshooting: "Add 'source $HOME/.cargo/env' to your ~/.bashrc or ~/.zshrc for permanent PATH setup"
          },
          {
            check: "Can run basic command",
            command: "near account view-account-summary --help",
            expectedResult: "Shows help text for the command",
            troubleshooting: "If error, try: source $HOME/.cargo/env and try again"
          }
        ],
      ...
      commonPitfalls: [
        {
          issue: "curl fails with SSL/TLS error",
          solution: "Check internet connection. If behind proxy, configure curl with --proxy flag"
        },
        {
          issue: "'near: command not found' after installation",
          solution: "Run: source $HOME/.cargo/env (this adds ~/.cargo/bin to PATH)"
        },
        {
          issue: "PATH not persisting in new shells",
          solution: "Add 'source $HOME/.cargo/env' to your ~/.bashrc or ~/.zshrc file"
        },
        {
          issue: "Permission denied when running installer",
          solution: "The installer creates ~/.cargo/bin - ensure $HOME is writable"
        },
        {
          issue: "Old version still running after update",
          solution: "Run: source $HOME/.cargo/env to refresh PATH, then verify with: which near"
        }
      ],
      
      aiAgentNote: "For AI agents: Always run 'source $HOME/.cargo/env' before using near-cli-rs commands in scripts. The installer adds ~/.cargo/bin to PATH, but this only takes effect in new shell sessions."
    },

    step2_createImplicitAccount: {
      title: "Step 2: Create Implicit Account",
      description: "Generate a new NEAR implicit account with Ed25519 keypair",
      
      prerequisite: "Make sure you've run 'source $HOME/.cargo/env' in your current shell session",
      
      commands: {
        createDirectory: "mkdir -p \"$HOME/.near-credentials/mainnet\"",
        generateAccount: "near account create-account fund-later use-auto-generation save-to-folder \"$HOME/.near-credentials/mainnet\""
      },

      expectedOutput: {
        message: "The file \"$HOME/.near-credentials/mainnet/<64-hex-account-id>.json\" was saved successfully",
        note: "The exact path depends on your $HOME directory (e.g., /home/username/ or /Users/username/)"
      },

      extractAccountId: {
        title: "Agent-readable: Extract account ID from the file path",
        script: `ACCOUNT_FILE=$(ls "$HOME/.near-credentials/mainnet"/*.json | head -1)
        ...
        checks: [
          {
            ...
            troubleshooting: "If no file found, re-run the account creation command"
          },
          {
            check: "File contains valid keypair",
            command: "cat $HOME/.near-credentials/mainnet/*.json | jq -r '.private_key, .public_key, .implicit_account_id'",
            expectedResult: "Three lines: private_key (ed25519:...), public_key (ed25519:...), and implicit_account_id (64 hex chars)",
...
echo "Private Key: $PRIVATE_KEY"`,
    ...
    step4_fundAccount: {
      ...
      verifyCurrentState: {
        title: "✅ How to Verify This Step Completed Successfully",
        description: "Use roditwallet.sh or near-cli-rs to check account balance",
        checks: [
          {
          ...
          {
            check: "Alternative: Check via near-cli-rs",
...
NONCE_TIMESTAMP=$(echo "$NONCE_RESPONSE" | jq -r '.timestamp')`
    ...
    commonErrorHandling: {
      ...
      errors: [
        ...
        {
          error: "command not found: near",
          cause: "near-cli-rs not in PATH",
          recoverySteps: [
            "1. Source cargo environment: source $HOME/.cargo/env",
            "2. Verify installation: $HOME/.cargo/bin/near --version",
            "3. Add to shell profile: echo 'source $HOME/.cargo/env' >> ~/.bashrc",
            "4. Retry command"
          ],
          verifyFix: "Command 'near --version' should show version number"
        },
        {
...
chmod 600 /home/agent/openclaw/near-credentials.json`
    ...
    automationTips: {
      ...
      tips: [
        ...
        "Keep near-cli-rs updated to latest stable version"
      ]
    }
  },
      ...
      subagentHolaFormat: {
        ...
        keyDifferences: [
          ...
          },
          {
            ...
            subagent: "publicKey - Your base64url-encoded Ed25519 public key (32 bytes)"
          },
          {
            field: "Fifth field",
        ...
        components: {
          ...
          publicKey: {
            format: "base64url-encoded Ed25519 public key (43 characters, no padding)",
        ...
        importantNotes: [
          ...
          "⚠️ The publicKey field is the SUBAGENT's public key (base64url-encoded)",
...
`
  ...
  // Managing RODiT Wallets with near-cli-rs
  roditWalletManagement: {
    title: "Managing RODiT Wallets with near-cli-rs",
    description: "Guide for managing RODiT tokens and NEAR accounts using the roditwallet.sh script and near-cli-rs",
    
    overview: {
      purpose: "Manage RODiT token ownership, transfers, and account initialization on NEAR blockchain",
      tool: "roditwallet.sh - A bash wrapper around near-cli-rs for RODiT operations",
      requirements: [
