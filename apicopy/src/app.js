...
const { probeNearRpcStatus } = require("./lib/near-rpc-probe");
...
function setupRoutes() {
  // Health check endpoint (liveness + cached dependency probes; 15s TTL)
  app.get("/health", async (req, res) => {
    const now = Date.now();
    if (healthChecksCache.payload && now - healthChecksCache.at < HEALTH_CHECKS_CACHE_MS) {
      res.status(200).json(healthChecksCache.payload);
      return;
    }
    try {
      const payload = await buildHealthPayload(req.app);
      healthChecksCache = { at: now, payload };
      res.status(200).json(payload);
    } catch (err) {
      logger.error("Health check dependency probe failed", {
        component: "IDClawserverAPI",
        error: err.message
      });
      res.status(200).json({
