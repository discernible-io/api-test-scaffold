  ...
  async getResource(uri, req) {
    ...
    if (uri === "onboarding:near") {
      const onboardingGuide = {
        ...
        description: "Step-by-step instructions for installing near-cli-rs-ai, obtaining NEAR tokens, and creating a NEAR implicit account. Required before minting a RODiT token via IDENTYCLAW.",
        sections: {
          install_near_cli_rs_ai: {
            title: "Install near-cli-rs-ai",
            description: "near-cli-rs-ai is the IdentityClaw fork of the NEAR Protocol CLI with AI agent optimizations. It is required for wallet management operations via roditwallet.sh.",
            methods: [
              {
                platform: "Linux (recommended .deb package)",
                command: "wget https://identyclaw.ams3.cdn.digitaloceanspaces.com/near-cli-rs-ai.deb && sudo dpkg -i near-cli-rs-ai.deb",
                notes: "Downloads and installs the pre-compiled .deb package. No Rust toolchain required."
              },
              {
                platform: "Build from Source",
                command: "git clone https://github.com/rodit-org/near-cli-rs-ai.git && cd near-cli-rs-ai && cargo build --release && sudo cp target/release/near /usr/bin/near",
                notes: "Requires Rust toolchain. Use this if the .deb package doesn't work for your architecture."
              }
            ],
            verify: {
              command: "near --version",
              expected: "Prints the near-cli-rs-ai version string, e.g. 'near-cli-rs-ai X.Y.Z'"
            }
          },
      ...
      try {
        const subagentsGuide = {
          ...
          holaFormats: {
            ...
            standard: {
              format: "HOLA/<recipient>/<tokenId>/<ISO8601-timestamp>/<noncets-hex>/API.IDENTYCLAW.COM/<base32-signature>/<checksum>",
              fields: 8,
            ...
            subagent: {
              format: "HOLA/<recipient>/<delegateID>/<issuer_tokenId>/<publicKey>/<ISO8601-timestamp>/<noncets-hex>/API.IDENTYCLAW.COM/<base32-signature>/<checksum>",
              fields: 11,
              ...
              additionalFields: [
                ...
                "publicKey - Subagent's Ed25519 public key (base32-encoded, 52 chars)"
              ]
            }
          },
            ...
            issues: [
              ...
              },
              {
                ...
                solutions: [
                  ...
                  "Count separators '/' - should be 7 for standard, 10 for subagent",
