...
router.post("/identity/verify", validateContentType, validateJsonBody, authenticate, async (req, res) => {
  ...
  if (!/^[a-z]{12}$/.test(tokenId)) {
    return sendError(res, {
      ...
      message: isSubagentFormat ? "issuerTokenId must be exactly 12 lowercase letters" : "tokenId must be exactly 12 lowercase letters"
    });
  }
  ...
  if (authenticatedTokenId && recipient !== authenticatedTokenId) {
    warnings.push({
    ...
    if (token && token.owner_id) {
      try {
        ...
        if (isSubagentFormat) {
          ...
          try {
            publicKeyBytes = new Uint8Array(Buffer.from(subagentPublicKey, "base64url"));
