...
**IMPORTANT RULE**: Only passing tests can be disabled by the test system. Not-passed tests can only be disabled by the user. The test system must never disable not-passed tests—they must be diagnosed and fixed.
...
**Logs**: Relevant log excerpts showing the not-passed outcome
**Required Fix**: Specific code changes needed in the API

The older a test module is, which you can see in git, the more you can trust that is properly implemented. Use this knowledge to compare test modules between them when one is not-passed, which often will be because is not following proper integration patterns with the SDK and how the test suite is engineered
